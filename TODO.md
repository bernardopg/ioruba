...
- [x] ~~Assinatura + notarização macOS~~ — **descartado por decisão de projeto** `(dist/macos/security)`
  - O projeto não possui e não pretende adquirir Apple Developer ID nem certificado Authenticode. Toda a ramificação condicional de assinatura foi removida do `release.yml`: um único passo de build, sem secrets `APPLE_*`/`WINDOWS_CERTIFICATE*`. A integridade segue verificável por `SHA256SUMS.txt` e por attestations do GitHub, e o updater in-app mantém assinatura própria (minisign), independente disso. Instalador e cask fazem strip de quarantine; o README documenta o aviso do SmartScreen.
  - Não reintroduzir passos condicionais: a ramificação anterior sobreviveu à remoção dos imports em `def4782` e deixou `WINDOWS_CERTIFICATE_THUMBPRINT`/`APPLE_SIGNING_IDENTITY` indefinidas — o workflow *parecia* assinar e não assinava.
- [ ] Gerar `.dmg` no macOS além do `.app.tar.gz` `(dist/macos)` - `médio`
  - **Não habilitado cegamente**: o script DMG do Tauri chama Finder via AppleScript, que já falhou por autorização intermitente em runner GitHub-hosted; adicioná-lo à matriz hoje faz uma release inteira falhar. O `.app.tar.gz` é o artefato reproduzível. Como a assinatura macOS foi descartada, só vale reabrir se o caminho de DMG se provar estável num draft — sem `codesign`/`spctl` para servir de gate.
