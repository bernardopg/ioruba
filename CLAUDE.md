...
- `npm run rust:audit` is **not** part of any GitHub workflow — it only runs via `npm run release:check` locally. GitHub-side Rust advisory coverage comes from Dependabot alerts, which do not flag `unmaintained` crates; `cargo audit` currently reports 16 such warnings (gtk-rs GTK3 and `unic-*`) and still exits 0. Run `npm run rust:audit` before cutting a release.
