...
impl LineRouter {
    pub fn route_streaming(
        ...
        while let Some(pos) = self.partial.find('\n') {
            // Keep the original line ending in ordinary stream data. Consumers
            // receive arbitrary serial chunks and commonly frame them on `\n`;
            // stripping it here concatenates frames into an unparseable stream.
            let raw_line = self.partial[..=pos].to_string();
            let line = raw_line
                .trim()
                .trim_end_matches('\r')
                .to_string();
            ...
            } else if is_likely_urc(&line, solicited_prefixes) && classify_final_line(&line).is_none() {
...
mod tests {
    ...
    #[test]
    fn line_router_preserves_stream_line_endings() {
        let mut router = LineRouter::default();
        let actions = router.route_streaming(b"512|768|1023\r\n0|1|2\n", &[]);
        let stream = actions
            .into_iter()
            .filter_map(|action| match action {
                RxRouteAction::StreamData(bytes) => Some(bytes),
                RxRouteAction::UrcLine(_) => None,
            })
            .flatten()
            .collect::<Vec<_>>();
