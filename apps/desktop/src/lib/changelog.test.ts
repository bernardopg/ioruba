...
describe("parseChangelog", () => {
  it("parses the real Keep a Changelog document", () => {
    ...
    expect(releases[0]?.version).toBe("1.7.0");
    expect(releases[0]?.date).toBe("2026-08-03");
    expect(releases[0]?.url).toContain("/compare/v1.6.3...v1.7.0");
    expect(releases[0]?.sections[0]?.title).toBe("Added");
    expect(releases[0]?.sections[0]?.items[0]).toContain("AudioTarget");
